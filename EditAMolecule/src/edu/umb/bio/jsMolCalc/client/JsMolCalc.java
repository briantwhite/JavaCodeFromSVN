...
public class JsMolCalc implements EntryPoint {
	...
	private HTML currentProperties;
	private ListBox taskChoiceList;
	...
	public void onModuleLoad() {
		...
		final HTML answerEvaluation = new HTML();
		final Button submitAnswerButton = new Button("Submit Answer");
		class CalculateButtonHandler implements ClickHandler {
			public void onClick(ClickEvent event) {
				...
				currentProperties.setHTML(result.info.toString());

				if (currentTarget.isCorrect(result.info)) {
					answerEvaluation.setHTML("<html><body><font color=\"green\">Correct!</font></body></html>");
					refreshTargets();
				} else {
					answerEvaluation.setHTML("<html><body><font color=\"red\">Incorrect!</font></body></html>");
				}
			}
		}
		CalculateButtonHandler cbHandler = new CalculateButtonHandler();
		submitAnswerButton.addClickHandler(cbHandler);

		currentProperties = new HTML();
		taskChoiceList = new ListBox();
		taskChoiceList.addChangeHandler(new ChangeHandler() {
			public void onChange(ChangeEvent event) {
				resetMolecule(molString);
				currentTarget = targets[taskChoiceList.getSelectedIndex()];
			}
		});

		RootPanel.get("submitAnswerButton").add(submitAnswerButton);
		RootPanel.get("currentProperties").add(currentProperties);
		RootPanel.get("answerEvaluation").add(answerEvaluation);
		RootPanel.get("taskChooser").add(taskChoiceList);

		try {
			new RequestBuilder(RequestBuilder.GET, "dopamine.mol").sendRequest("", new RequestCallback() {
				public void onResponseReceived(Request req, Response resp) {
					molString = resp.getText();
					setJMEmol(molString);
					InfoAndTargets result = computeAndDisplay(getJMEmol(), getJMESmiles(), getJMEjme());
					currentProperties.setHTML(result.info.toString());
					...
					for (int i = 0; i < targets.length; i++) {
						taskChoiceList.addItem(targets[i].toString());
					}
					currentTarget = targets[0];
					answerEvaluation.setHTML("<html><body>Ready</body></html>");
				}
	...
	private void resetMolecule(String molString) {
		setJMEmol(molString);
		InfoAndTargets result = computeAndDisplay(getJMEmol(), getJMESmiles(), getJMEjme());
		currentProperties.setHTML(result.info.toString());
	}
	...
	private void refreshTargets() {
		taskChoiceList.clear();
		for (int i = 0; i < targets.length; i++) {
			if (targets[i].isCompleted()) {
				taskChoiceList.addItem(targets[i].toString() + " COMPLETED");	
			} else {
				taskChoiceList.addItem(targets[i].toString());	
			}
		}
	...
	InfoAndTargets computeAndDisplay(String molString, String smileString,
		...
		if (molString.equals("") || smileString.equals("") || jmeString.equals("")) {
			return new InfoAndTargets(null, null);
		}
		...
		if (errorString.equals("")) {
			...
			if (canMakeHbonds) {
				targets[0] = new Target(Target.NO_HBONDS);
			} else {
				targets[0] = new Target(Target.YES_HBONDS);				
			}

			if (canMakeIonicBonds) {
				targets[1] = new Target(Target.NO_IONIC_BONDS);
			} else {
				targets[1] = new Target(Target.YES_IONIC_BONDS);				
			}

			targets[2] = new Target((logp - 1.5f), (logp - 0.5f));

			targets[3] = new Target((logp + 0.5f), (logp + 1.5f));
		} 
		return new InfoAndTargets(info, targets);
	}
