...
C------------------------------------------------------------------
	...
	public static long getNumMutations(int popSize, float mutRate, Random r) {
		long x = Math.round((getInvCDF(r.nextDouble(), false) * popSize * mutRate * (1 - mutRate)) + (popSize * mutRate));
		while (x < 0) {
			x = Math.round((getInvCDF(r.nextDouble(), false) * popSize * mutRate * (1 - mutRate)) + (popSize * mutRate));
		}
		return x;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("Computing " + numRuns + " values where pop=" + popSize + " and mut rate=" + mutRate);
		Random r = new Random();
		for (int i = 0; i < numRuns; i++) {
			System.out.println(getNumMutations(popSize, mutRate, r));
		}
	}
}