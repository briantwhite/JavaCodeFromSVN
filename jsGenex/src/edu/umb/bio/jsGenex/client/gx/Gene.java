...
public class Gene {
	...
	public Gene(String inputDNASequence,
		...
		_promoterSpacing = promoterSpacing;
	...
	public void transcribe() {
		...
		/*
		 * see if there's a promoter
		 *   if not, make nothing
		 *   if yes, look for terminator
		 *   	if yes, stop at terminator
		 *      if no, go to end of DNA
		 */
		if (promoterSite != -1) {  
			int mRNANucleotideNumber = 0;
			//set the values for this gene
			_promoterStart = promoterSite;
			_numSpacesBeforeRNA_Start = _promoterStart + _promoterSequence.length() + _promoterSpacing;

			if (terminatorSite != -1) {
				_terminatorStart = terminatorSite;
			} else {
				_terminatorStart = _DNASequence.length();
			}

			//mark nucleotides in the pre mRNA from promoter to terminator
			for (int i = _numSpacesBeforeRNA_Start; i < _terminatorStart; i++) {
	...
	public String generateDNA_HTML(int selectedBase){
		...
		ColorBuffer.append(ColorTopStrandBuffer.toString() + "</EM></span>-3\'\n"
