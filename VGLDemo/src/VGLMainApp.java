 ...
 * @version 1.0 $Id: VGLMainApp.java,v 1.1 2005-06-01 13:30:44 brian Exp $
 */
public class VGLMainApp extends JApplet {
	...
	/**
	 * Menu item to invoke instructions
	 */
	private JMenuItem m_instructionsItem = null;
	...
	private void menuBar() {
		...
		m_instructionsItem = menuItem("Show Instructions", "Instructions",null);
		mnuHelp.add(m_instructionsItem);
	...
	private void newProblem() {
		String problemType = new String("");
		if (m_CageCollection == null) {
			String[] typeFiles = {"level01","level02","level03",
					              "level04","level05","level06",
								  "level07","level08","level09","level10"};
			JList typeFileList = new JList(typeFiles);
			typeFileList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			typeFileList.setSelectedIndex(1);
			int ans1 = JOptionPane.showConfirmDialog(this,
					typeFileList,
					"Choose a new Problem Type", JOptionPane.OK_CANCEL_OPTION,
					JOptionPane.QUESTION_MESSAGE);
			if (ans1 == JOptionPane.OK_OPTION){
				problemType = (String)typeFileList.getSelectedValue();
	    		if (problemType != null) {
	    			newProblem(problemType + ".prb");
	    		}
			}
		}
	}
	...
	/**
	 * Method to set up a new problem for the user from a preset problem file
	 */
	private void newProblem(String problemFileName) {
		if (m_CageCollection == null) {
			URL newFileURL = null;
			try {
				if (problemFileName.indexOf(System.getProperty("file.separator")) == -1){
					newFileURL = VGLMainApp.class.getResource("Problems/" 
							+ problemFileName);
				} else {
					newFileURL = new URL(problemFileName);
				}
			...
			if (newFileURL != null) {
				...
				m_Genetics = new Genetics(newFileURL);
	...
	/**
	 * This method invokes .html instructions into a JEditor pane
	 */
	private void showInstructions() {
		final JEditorPane instructionPane = new JEditorPane();
		instructionPane.setEditable(false);
		instructionPane.setContentType("text/html");
		
		try {
			instructionPane.setPage(VGLMainApp.class.getResource(
					"instructions/instructions.html"));
		} catch (Exception e) {
			JOptionPane.showMessageDialog(m_DialogFrame,
					"Be sure the instructions folder is in the same folder as VGL.",
					"Can't find instructions file.", JOptionPane.ERROR_MESSAGE);
			return;
		}

		JScrollPane instructionsScrollPane = new JScrollPane(instructionPane);
		JDialog helpDialog = new JDialog(m_DialogFrame, "VGL Instructions");
		JButton backButton = new JButton("Go Back to Top of Page");
		helpDialog.getContentPane().setLayout(new BorderLayout());
		helpDialog.getContentPane().add(backButton, BorderLayout.NORTH);
		helpDialog.getContentPane().add(instructionsScrollPane, BorderLayout.CENTER);
		Dimension screenSize = getToolkit().getScreenSize();
		helpDialog.setBounds((screenSize.width * 55/100), (screenSize.height / 8),
				(screenSize.width * 45/100), (screenSize.height * 8 / 10));
		helpDialog.show();

		instructionPane.addHyperlinkListener(new HyperlinkListener() {
			public void hyperlinkUpdate(HyperlinkEvent e) {
				if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					try {
						instructionPane.setPage(e.getURL());
					} catch (IOException ioe) {
						System.err.println(ioe.toString());
					}
				}
