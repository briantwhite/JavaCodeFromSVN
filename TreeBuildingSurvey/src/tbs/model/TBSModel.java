...
public abstract class TBSModel {
	...
	public int getSerial() {
		lastUsedIdNumber++;
		System.out.println(lastUsedIdNumber);
		return lastUsedIdNumber;
	}

	/**
	 * update serial number to highest last used value
	 * 	used when loading trees to be sure that
	 * 	anything you add has a safe serial number.
	 */
	public void updateLastUsedIDNumber(int serialNumber) {
		if (serialNumber > lastUsedIdNumber) {
			lastUsedIdNumber = serialNumber;
		}
	}
	...
	public void refreshSerial() {
		lastUsedIdNumber = elements.size();
	}
	...
	throws NumberFormatException {
		...
		if (fromIndex >= 0 && toIndex >= 0) {
			...
			tempTree.add(new Connection(id, fromNode, toNode));	//used getSerial() originally (BW)
			updateLastUsedIDNumber(id);
		}
	}
