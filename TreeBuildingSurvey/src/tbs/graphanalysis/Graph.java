...
import tbs.graphanalysis.Vertex.Mark;
...
public class Graph implements Renderable {
	...
	/*
	 * added by BW 
	 * 	need to test for weird trees
	 * 		those with unconnected branch points 
	 * 		those with branch points at ends of lines
	 */
	public boolean hasUnconnectedBranchPoints() {
		for (Vertex v : vertices) {
			if ((v.getType() == VertexInfo.VertexType.EMPTY) 
					&& (v.getAdjVertices(false).size() == 0)) {
				return true;
			}
		}
		return false;
	...
	public boolean hasTerminalBranchPoints() {
		for (Vertex v : vertices) {
			if ((v.getType() == VertexInfo.VertexType.EMPTY) 
					&& (v.getAdjVertices(false).size() == 1)) {
				return true;
			}
		}
		return false;
	...
	/*
	 * Added by BW
	 * see if all organisms are connected together
	 * so you can walk to all 20 starting from Bat
	 * 
	 * algorithm
	 * 		white = not visited
	 * 		black = visited
	 * 
	 * start by marking all white
	 * then pick an arbitrary starting point
	 * 		vertices(0) = Bat
	 * and see if you can get to all of them or not
	 * 
	 * use vertex.getAdjVertices(false) - to get all connected
	 * 		vertices, regardless of direction of link
	 * if you do this, you have to be careful not to come back
	 * 		to starting vertex -> infinite loop
	 * fix by, if it has neighbors, you mark it black at start
	 * 
	 * it returns an integer where each bit is 1 if that organism was visited
	 * since 20 organisms = 20 bits
	 * max bit is 2^19
	 * so if all hit, the value is 2^20 - 1
	 * 
	 */
	public boolean allOrganismsConnected() {
		/*
		 * try starting with Bat and see how many you can visit
		 * via links
		 * first, see if they used bat at all
		 */
		if (vertices.size() == 0) return false;		// no organisms used
		if (membersOfSameTreeAs(vertices.get(0)) == (Math.pow(2,20) - 1)) {
			return true;
		} else {
			return false;
		}
	...
	public int membersOfSameTreeAs(Vertex startingVertex) {
		// set all to 'unvisited'
		for (Vertex v : vertices) {
			v.setMark(Mark.WHITE);
		}

		/*
		 * if the starting vertex has neighbors make it black, so you won't
		 *    come back to it during your search
		 */
		if (startingVertex.getAdjVertices(false).size() != 0) {
			startingVertex.setMark(Mark.BLACK);
		}
		explore(startingVertex);

		// see if you got them all
		// ignore "empty vertices" - these are branch points
		int visitedVertexSet = 0;
		for (Vertex v : vertices) {
			if ((v.getMark() == Mark.BLACK) && (v.hasName())) { 
				visitedVertexSet = visitedVertexSet | (int)Math.pow(2, v.getIndex());
			}
		}
//		System.out.println(Integer.toBinaryString(visitedVertexSet));
//		System.out.println(integerToOrgList(visitedVertexSet));
		return visitedVertexSet;
	}

	/*
	 * this is for allOrgsConnected
	 * it visits all links regardless of direction
	 * so it must mark starting vertex to prevent
	 * infinite loop
	 */
	private void explore(Vertex v) {
		for (Vertex v2 : v.getAdjVertices(false)) {
			if (v2.getMark() == Mark.WHITE) {
				if (v.getAdjVertices(false).size() != 0) v.setMark(Mark.BLACK);
				explore(v2);

				/*
				 * once you finish with neighbors, it's a terminal node
				 * so mark it done
				 * only bother if it's white
				 */
				if (v2.getMark() == Mark.WHITE) v2.setMark(Mark.BLACK);
			} 
		}
	}
	
	private String integerToOrgList(int i) {
		StringBuffer inSet = new StringBuffer();
		int in = 0;
		StringBuffer notInSet = new StringBuffer();
		int notIn = 0;
		for (Vertex v : vertices) {
			if ((v.getMark() == Mark.BLACK) && (v.hasName())) { 
				 if ((i & (int)Math.pow(2, v.getIndex())) != 0) {
					 inSet.append("\t" + v.getName() + "\n");
					 in++;
				 } else {
					 notInSet.append("\t" + v.getName() + "\n");
					 notIn++;
				 }
			}
		}
		return "In Set: " + in + "\n" + inSet.toString() + "Not in Set: " + notIn + "\n" + notInSet.toString();
	}
