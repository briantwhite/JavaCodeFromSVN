...
public class FoldingManager {
	...
	public GridCanvas createCanvas() {
		...
		gridCanvas.setGrid(currentGrid);
		return gridCanvas;
	}
	...
	public void createCanvas(OutputPalette outputPanel) {
		//TO DO outputPanel.getDrawingPane().repaint();
		//TO DO outputPanel.getDrawingPane().setGrid(currentGrid);
	}
	...
	private void foldPP(Attributes attrib) 
		...
		createGrid(attrib);
		createFolder(attrib);
		//((IncrementalFolder)currentFolder).addProgressEventListener(this.protexGWT);
		currentFolder.fold();
	}
	...
	private void createFolder(Attributes attrib) throws FoldingException {
		...
		if (folder.equalsIgnoreCase("bruteforce")) {
			...
			((BruteForceFolder)currentFolder).protexGWT = protexGWT;
