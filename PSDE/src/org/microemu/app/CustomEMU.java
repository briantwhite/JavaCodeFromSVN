...
public class CustomEMU extends JFrame {
	...
	private ActionListener menuOpenJADFileListener = new ActionListener() {
		public void actionPerformed(ActionEvent ev) {
			...
			int returnVal = fileChooser.showOpenDialog(CustomEMU.this);
	...
	private ActionListener menuSaveForWebListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (saveForWebChooser.showSaveDialog(CustomEMU.this) == JFileChooser.APPROVE_OPTION) {
				...
				if (appletJarFile == null) {
					...
					if (appletChooser.showOpenDialog(CustomEMU.this) == JFileChooser.APPROVE_OPTION) {
		...
		private boolean allowOverride(File file) {
			if (file.exists()) {
				int answer = JOptionPane.showConfirmDialog(CustomEMU.this, "Override the file:" + file + "?", "Question?",
	...
	private ActionListener menuStartCaptureListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (captureFileChooser.showSaveDialog(CustomEMU.this) == JFileChooser.APPROVE_OPTION) {
	...
	private ActionListener menuRecordStoreManagerListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if (recordStoreManagerDialog == null) {
				recordStoreManagerDialog = new RecordStoreManagerDialog(CustomEMU.this, common);
	...
	private ActionListener menuLogConsoleListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if (logConsoleDialog == null) {
				logConsoleDialog = new SwingLogConsoleDialog(CustomEMU.this, CustomEMU.this.logQueueAppender);
	...
	private ActionListener menuAboutListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			SwingDialogWindow.show(CustomEMU.this, "About", new SwingAboutDialog(), false);
		}
	};
	...
	private ActionListener menuSelectDeviceListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if (SwingDialogWindow.show(CustomEMU.this, "Select device...", selectDevicePanel, true)) {
				if (selectDevicePanel.getSelectedDeviceEntry().equals(deviceEntry)) {
					return;
				}
				int restartMidlet = 1;
				if (MIDletBridge.getCurrentMIDlet() != common.getLauncher()) {
					restartMidlet = JOptionPane.showConfirmDialog(CustomEMU.this,
	...
	public CustomEMU(DeviceEntry defaultDevice) {
		...
		this.resizeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				...
				resizeDeviceDisplayDialog.setDeviceDisplaySize(deviceDisplay.getFullWidth(), deviceDisplay
						.getFullHeight());
				if (SwingDialogWindow.show(CustomEMU.this, "Enter new size...", resizeDeviceDisplayDialog, true)) {
