...
public class FoldingManager {
	...
	/**
	 * Add a FoldingObserver to the list of observer objects.
	 * 
	 * @param o
	 *            FoldingObserver.
	 */
	public void attach(FoldingObserver obs) {
		if (observers.indexOf(obs) < 0) {

			if (DEBUG) {
				System.out.print("\nFoldingManager.attach(): ");
				System.out.println("Adding " + obs.toString()
						+ " to the list of observer objects");
			}
	...
	/**
	 * Remove a FoldingObserver from the list of observer objects.
	 * 
	 * @param o
	 *            FoldingObserver.
	 */
	...
	public void fold(Attributes attrib) throws FoldingException {
		...
		foldPP(attrib);
//		attrib.saveState("outfile"); // TODO implement attrib.saveState

		// observers:
		//		ProtexMainApp [and ProtexAdminGUI]
		notifyObservers();
	}
	...
	private void createPP(Attributes attrib) 
		...
		try {
			currentPP = factory.createPolypeptide(
				...
				attrib.getTable(),
				attrib.getPpId());
			currentPP.setUHL(null, attrib.getHistory(), null);
		} 
	...
	private void notifyObservers() {
		...
		while (iter.hasNext()) {
			...
			if (DEBUG) {
				System.out.print("\nFoldingManager.notifyObservers(): ");
				System.out.println("Sending a message to " + obs.toString());
			}

			obs.doneFolding(currentAttrib);
		}
	}
}
